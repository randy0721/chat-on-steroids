...
export const pluginCatalog: PluginCatalogEntry[] = [
  ...
  },
  {
    ...
    license: 'MIT',
    fields: [
      { key: 'MYSQL_HOST', label: 'MySQL host', required: true, placeholder: 'Example: 127.0.0.1 or db.example.internal' },
      { key: 'MYSQL_PORT', label: 'MySQL port', placeholder: '3306' },
      { key: 'MYSQL_DATABASE', label: 'Default database', placeholder: 'Optional' },
      { key: 'MYSQL_USER', label: 'MySQL user', required: true, placeholder: 'Use a read-only database account' },
      { key: 'MYSQL_PASSWORD', label: 'MySQL password', secret: true, placeholder: 'Stored in secure storage' },
    ],
    tools: ['list_databases', 'list_tables', 'describe_table', 'execute_query'],
    instructions: [
      'Install Node.js 18+.',
      'Use a dedicated read-only MySQL account with access only to the databases this plugin should inspect.',
      'This preset is read-only: its query tool accepts SELECT, SHOW, DESCRIBE and EXPLAIN statements and blocks data-changing SQL.',
    ],
  },
  {
    id: 'playwright',
