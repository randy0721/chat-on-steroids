...
Playwright MCP's pinned tarball supplies Apache-2.0 LICENSE and no standalone NOTICE. MySQL MCP's pinned npm tarball and Blender, Fetch and Unity's pinned distributions supply MIT license files and no standalone NOTICE. These observations describe the inspected direct server distributions, not an exhaustive transitive dependency audit. The rejected Context7, Exa, Firecrawl and Tavily recipes and their direct-package license snapshots were removed from the current inventory.
